# -*- coding: utf-8 -*-
"""
Created on Thu Mar 13 07:01:29 2025
...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 11:05:23 2025
...
class PaperTradeExecutor:
    ...
    def place_commodity_order(self, symbol, action, quantity, price_type="MARKET", price=0):
        """
        Place an order for a commodity
        ...
        dict
            Order result
        """
        # Convert to AlgoMojo format
        algomojo_symbol = self.symbol_registry.map_symbol(symbol, "tv", "algomojo")
        
        # Place order with MCX exchange
        return self.place_order(
            symbol=algomojo_symbol,
            action=action,
            quantity=quantity,
    ...
    def place_commodity_option_order(self, underlying, option_type, quantity, price_type="MARKET", price=0):
        """
        Place an option order for a commodity
        
        Parameters:
        -----------
        underlying : str
            Underlying commodity symbol (e.g., "CRUDEOIL")
        option_type : str
            "CE" or "PE"
        quantity : int
            Order quantity
        ...
        # Get symbol info
        symbol_info = None
        for info in self.symbol_registry.get_all_symbols():
            if info.get("symbol") == underlying and info.get("type") == "commodity":
                symbol_info = info
                break
        ...
        price = 1000.0  # In a real implementation, fetch actual price
        
        # Place the option order
        return self._place_option_order(symbol_info, option_type, price, quantity)